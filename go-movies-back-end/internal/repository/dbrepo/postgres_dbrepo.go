...
type PostgresDBRepo struct {
	DB *sql.DB // It is holding all our database connections
}


  const dbTimeout = time.Second * 3 // We are giving three seconds to interact with the database  and if it takes longer
   // than 3 seconds  we are going to cancel  that request....
 ...
 func (m *PostgresDBRepo) AllMovies(genre ...int) ([]*models.Movie , error) {
   ...
   var movies []*models.Movie
    ...
    for rows.Next() { // Go through each of the rows one at a time....
    
		 var movie models.Movie // This is a variable movie of type models.Movie

		 err := rows.Scan( // This will deal with only the current row that we are looking at...
            &movie.ID,
			&movie.TITLE,
			&movie.ReleaseDate,
			&movie.RunTime,
  ...
  func (m *PostgresDBRepo) GetUserByID(id int) (*models.User , error) {
	...
	query := `select id , email , first_name , last_name , password
	          createdAt , updatedAt from users where id = $1`
